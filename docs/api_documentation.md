...
## Historique des routes V17/V18 archivées

Les routers expérimentaux V17/V18 ont été retirés de `backend/app/main.py` après calcul de la fermeture de dépendances et validation de la suite backend.

Routes retirées du runtime actif :

- `/api/experimental/ml/v17-8/*` ;
- `/api/experimental/ml-clubs/v17-8/*` ;
- `/api/experimental/ml-national/v18-3-3/*`.

Le code historique correspondant est conservé sous `archive/backend_legacy/v17_v18/` afin de préserver la traçabilité RNCP. Les preuves, datasets et modèles V17/V18 restent conservés dans leurs dossiers métier.

Deux briques historiques restent actives parce que V19 les utilise encore :

- `backend/app/services/ml_clubs_v17_8_feature_builder.py` ;
- `backend/app/services/ml_v17_8_service.py`.

Les endpoints produit actuellement actifs pour la génération V19 sont :

- `GET /api/experimental/ml-v19/rubybets-matches/{match_id}` ;
- `POST /api/experimental/ml-v19/selection` ;
- `GET /api/experimental/ml-v19/h2h/rubybets-matches/{match_id}`.

Validation après archivage : **285 tests backend passés**.
